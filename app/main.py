...
# CORS: Nur erlaubte Origins (Flutter-App, lokale Entwicklung).
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
...
async def api_key_middleware(request: Request, call_next):
    ...
    if API_KEY:
        ...
        if key != API_KEY:
            return JSONResponse(
                status_code=401,
                content={"detail": "Ungültiger oder fehlender API-Key"},
            )
...
app.include_router(transcribe.router,  prefix="/voice",           tags=["voice"])
